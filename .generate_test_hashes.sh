...
formatted_share_link="$(echo -ne "# this is a test\n## of backend rendering\n\n" | curl -s --data-binary @/dev/stdin $DEPLOY_URL | grep share | awk '{print $4}' | sed -e 's/\&raw//g')"
rendered_sha256=$(curl -s $formatted_share_link | sed $NO_ULID | sed $NO_EXPIRY | sed $NO_TWITTER | sed $NO_URL | bare_sha256sum)
...
image_share_link="$(curl -s --data-binary @deps/notpacman.png $DEPLOY_URL | grep share\ link | awk '{print $4}' | sed -e's/\&raw//g')"
image_embed_sha256=$(curl -s $image_share_link | sed $NO_ULID | sed $NO_EXPIRY | sed $NO_TWITTER | sed $NO_URL | bare_sha256sum)
...
echo "curl -s '$formatted_share_link' | sed '$NO_ULID' | sed '$NO_EXPIRY' | $NO_TWITTER | $NO_URL | sha256sum"
echo ""
